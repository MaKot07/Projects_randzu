...
class Analitycs:
    ...
    def find_near_chips(self, x, y, need_color):
        ...
        for i in range(-1, 2):
            for j in range(-1,2):
                new_x = x + i
                new_y = y + j
                if [new_x, new_y, need_color] in self.now_coord_all_move_and_color:
                    near_chips.append([new_x, new_y])
    ...
    def check_motion(self, x, y):
        ...
        if x <= (cell_qty * cell_size_ramka) and x >= 0 and y <= (cell_qty * cell_size_ramka) and y >= 0:
            if [x_rect, y_rect, white] not in self.now_coord_all_move_and_color and [x_rect, y_rect, black] not in self.now_coord_all_move_and_color:
                return True
        return False
    ...
    def check_motion_for_generator(self, x, y):
        ...
        if x <= cell_qty and x >= 0 and y <= cell_qty and y >= 0:
            if [x, y, white] not in self.now_coord_all_move_and_color and [x, y, black] not in self.now_coord_all_move_and_color:
                return True
